...
namespace mjpc
{
    ...
    // get a single numerical value from a custom element in mjModel, or return
    // the default value if a custom element with the specified name does not
    // exist
    template <typename T>
    T GetNumberOrDefault(T default_value, const mjModel *m,
    ...
    // get the string selected in a drop down with the given name, given the
    // value in the residual parameters vector
    ...
    // returns a default value to put in residual parameters, given the index of
    // a custom numeric attribute in the model
    ...
    void LogScale(double *values, double max_value, double min_value,
                  int steps);

    // get a pointer to a specific element of a vector, or nullptr if out of
    // bounds
    template <typename T>
    inline T *DataAt(std::vector<T> &vec,
    ...
    // in C++20 atomic::operator+= is built-in for floating point numbers, but
    // this function works in C++11
    ...
    public:
        ...
        // compute Jacobian
        void
        Compute(std::function<void(double *output, const double *input)> func,
    ...
    void ConditionMatrix(double *res, const double *mat, double *mat00,
                         ...
                         double *bandfactor = NULL, int nband = 0);
