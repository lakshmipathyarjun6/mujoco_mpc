...
namespace mjpc
{
    ...
    ResidualSelectionLists(const mjModel *m)
    {
        ...
        {
            if (!absl::StartsWith(
                    std::string_view(m->names + m->name_textadr[i]),
                    "residual_list_"))
            {
                continue;
            }
                                  ...
                                  double residual_parameter)
    {
        ...
        // we're using a double field to store an integer - reinterpret as an
        // int
        ...
        for (int i = 0; i < m->ntext; i++)
        {
            ...
            {
                // get the nth element in the list of options (without
                // constructing a vector<string>)
                std::string_view options(m->text_data + m->text_adr[i]);
    ...
    double ResidualParameterFromSelection(const mjModel *m,
    ...
    void CheckSensorDim(const mjModel *model, int residual_size)
    {
        ...
        for (int i = 0; i < model->nsensor; i++)
        {
            if (model->sensor_type[i] == mjSENS_USER)
            {
                ...
                {
                    mju_error("user type sensors must come before other sensor "
                              "types");
                }
            }
            else
    ...
    {
        // get bounds
        auto it = std::upper_bound(sequence.begin(), sequence.begin() + length,
                                   value);
    ...
    void ZeroInterpolation(double *output, double x,
    ...
    void CubicInterpolation(double *output, double x,
                            const std::vector<double> &xs, const double *ys,
                            int dim, int length)
    {
        // find interval
        ...
        for (int i = 0; i < dim; i++)
        {
            ...
            double m0 =
                FiniteDifferenceSlope(xs[bounds[0]], xs, ys, dim, length, i);
            double m1 =
                FiniteDifferenceSlope(xs[bounds[1]], xs, ys, dim, length, i);
    ...
    std::string GetExecutableDir()
    {
        ...
        std::string realpath = [&]() -> std::string
        {
            ...
            while (!success)
            {
                ...
                {
                    std::cerr
                        << "cannot allocate memory to store executable path\n";
                    return "";
                }

                DWORD written =
                    GetModuleFileNameA(nullptr, realpath.get(), buf_size);
                if (written < buf_size)
                {
                    success = true;
                }
    ...
    void StateDiff(const mjModel *m, mjtNum *ds, const mjtNum *s1,
                   const mjtNum *s2, mjtNum h)
    {
                  ...
                  const mjtByte *geomgroup)
    {
        ...
        const mjtNum height_offset =
            .5;                       // add some height in case of penetration
        ...
        const mjtByte *query_geomgroup =
            geomgroup ? geomgroup : default_geomgroup;
        mjtNum dist = mj_ray(model, data, query, down, query_geomgroup,
                             flg_static, bodyexclude, &geomid);
                   ...
                   const double foot3[3])
    {
        ...
        if (mju_dot3(zaxis, to_body) < 0)
        {
            ...
            for (const int i : {1, 2, 4, 5, 7, 8})
                mat[i] *= -1;
        }

        // copy outputs
        if (feet_pos)
            mju_copy3(feet_pos, pos);
        if (feet_mat)
            mju_copy(feet_mat, mat, 9);
        if (feet_quat)
            mju_mat2Quat(feet_quat, mat);
    }
                      ...
                      const float rgba[4])
    {
        ...
        scene->geoms[scene->ngeom].category = mjCAT_DECOR;
        mjv_makeConnector(&scene->geoms[scene->ngeom], type, width, from[0],
...
// number of available hardware threads
    ...
    int NumAvailableHardwareThreads(void)
    {
        // start by assuming a maximum of 128 hardware threads and keep growing
        // until the cpu_set_t is big enough to hold the mask for the entire
        // machine
    ...
    { // private functions in an anonymous namespace
        ...
        {
            mjtNum current_edge[2] = {next[0] - current[0],
                                      next[1] - current[1]};
            ...
            else if (abs(projection) < mjMINVAL)
            {
                ...
                mjtNum candidate_length2 =
                    mju_dot2(candidate_edge, candidate_edge);
        ...
        // returns 2D point on line segment from v0 to v1 that is nearest to
        // query point
                       ...
                       const mjtNum *points, const int *hull, int num_hull)
    {
        ...
        for (int i = 0; i < num_hull; i++)
        {
            ...
            // if constraint is active, project to the edge, compare to best so
            // far
            ...
            for (int candidate = 0; candidate < num_points; candidate++)
            {
                if ((next == -1) ||
                    IsEdgeOutside(points + 2 * hull[num_hull - 1],
                                  points + 2 * next, points + 2 * candidate))
                {
                    next = candidate;
                }
            }
    ...
    {
        // allocate memory
        if (dim != gradient.size())
            gradient.resize(dim);
        if (dim != workspace_.size())
            workspace_.resize(dim);
    }

    // compute finite-difference gradient
    void FiniteDifferenceGradient::Compute(
        std::function<double(const double *x)> func, const double *input,
        int dim)
    {
        // resize
        Resize(dim);
    ...
    void FiniteDifferenceJacobian::Resize(int num_output, int num_input)
    {
        ...
        if (output_nominal.size() != num_output)
            output_nominal.resize(num_output);
        if (workspace_.size() != num_input)
    ...
    {
        // resize
        if (dim * dim != hessian.size())
            hessian.resize(dim * dim);
        if (dim != workspace1_.size())
            workspace1_.resize(dim);
        if (dim != workspace2_.size())
            workspace2_.resize(dim);
        if (dim != workspace3_.size())
    ...
    // set scaled block (size: rb x cb) in mat (size: rm x cm) given mat upper
    // row and left column indices (ri, ci)
    void SetBlockInMatrix(double *mat, const double *block, double scale,
                          int rm, int cm, int rb, int cb, int ri, int ci)
    {
    ...
    // set scaled block (size: rb x cb) in mat (size: rm x cm) given mat upper
    // row and left column indices (ri, ci)
    void AddBlockInMatrix(double *mat, const double *block, double scale,
                          int rm, int cm, int rb, int cb, int ri, int ci)
    {
        // loop over block rows
        for (int i = 0; i < rb; i++)
    ...
    void BlockFromMatrix(double *block, const double *mat, int rb, int cb,
                         int rm, int cm, int ri, int ci)
    {
        // loop over block rows
        for (int i = 0; i < rb; i++)
        {
    ...
    void DifferentiateSubQuat(double jaca[9], double jacb[9],
                              const double qa[4], const double qb[4])
    {
        // compute 3D velocity
        double axis[3];
                                       ...
                                       const double *qpos1, const double *qpos2)
    {
        ...
        // zero Jacobians
        if (jac1)
            mju_zero(jac1, model->nv * model->nv);
        if (jac2)
            mju_zero(jac2, model->nv * model->nv);
        ...
        for (int j = 0; j < model->njnt; j++)
        {
            ...
            case mjJNT_FREE:
                for (int i = 0; i < 3; i++)
                {
                    // qvel[vadr + i] = (qpos2[padr + i] - qpos1[padr + i]) /
                    // dt;
                    if (jac1)
                        jac1[(vadr + i) * model->nv + vadr + i] = -1.0 / dt;
                    if (jac2)
                        jac2[(vadr + i) * model->nv + vadr + i] = 1.0 / dt;
                }
                vadr += 3;
                padr += 3;
            ...
            case mjJNT_BALL:
                // mju_negQuat(neg, qpos1 + padr);  // solve:  qpos1 * dif =
                // qpos2 mju_mulQuat(dif, neg, qpos2 + padr); mju_quat2Vel(qvel
                // + vadr, dif, dt);
                ...
                if (jac1)
                {
                    ...
                    double jac1_blk[9];
                    DifferentiateSubQuat(NULL, jac1_blk, qpos2 + padr,
                                         qpos1 + padr);

                    // set block in Jacobian
                    SetBlockInMatrix(jac1, jac1_blk, 1.0 / dt, model->nv,
                                     model->nv, 3, 3, vadr, vadr);
                }
                ...
                if (jac2)
                {
                    // compute subQuat Jacobian block
                    double jac2_blk[9];
                    DifferentiateSubQuat(jac2_blk, NULL, qpos2 + padr,
                                         qpos1 + padr);

                    // set block in Jacobian
                    SetBlockInMatrix(jac2, jac2_blk, 1.0 / dt, model->nv,
                                     model->nv, 3, 3, vadr, vadr);
                }
            ...
            case mjJNT_SLIDE:
                ...
                if (jac1)
                    jac1[vadr * model->nv + vadr] = -1.0 / dt;
                if (jac2)
                    jac2[vadr * model->nv + vadr] = 1.0 / dt;
            }
        }
    }
        ...
        for (int i = 0; i < num_blocks; i++)
        {
            ...
            for (int j = i; j < i + num_cols; j++)
            {
                ...
                if (j > i)
                {
                    ...
                    AddBlockInMatrix(res, tmp2, 1.0, ntotal, ntotal, dblock,
                                     dblock, (j + res_start_col) * dblock,
                                     (i + res_start_row) * dblock);
                }
            }
        }
    ...
    void DenseToBlockBand(double *res, int dim, int dblock, int nblock)
    {
        ...
        for (int i = 0; i < num_blocks; i++)
        {
            ...
            {
                ZeroBlockInMatrix(res, dim, dim, dblock, dblock, i * dblock,
                                  j * dblock);
                if (j > i)
    ...
    void ConditionMatrix(double *res, const double *mat, double *mat00,
                         double *mat10, double *mat11, double *tmp0,
                         double *tmp1, int n, int n0, int n1,
                         double *bandfactor, int nband)
    {
        // unpack mat
        ...
        if (nband > 0 && bandfactor)
        {
            ...
            for (int i = 0; i < n1; i++)
            {
                mju_cholSolveBand(tmp0 + n0 * i, bandfactor, mat10 + n0 * i, n0,
                                  nband, 0);
            }
        }
        else
    ...
    void SetBlockInBand(double *band, const double *block, double scale,
                        int ntotal, int nband, int nblock, int shift,
                        int row_skip, bool add)
    {
        // loop over block rows
        for (int i = row_skip; i < nblock; i++)
        {
            ...
            // row segments
            double *band_row =
                band + (shift + row_skip + i) * nband + column_shift;
