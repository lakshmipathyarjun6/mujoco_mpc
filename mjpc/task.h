...
namespace mjpc
{
    ...
    public:
        ...
        // Must be called whenever parameters or weights change outside
        // Transition or Reset, so that calls to Residual use the new
        // parameters. Calls InternalResidual()->Update() with a lock.
        void UpdateResidual();

        // Changes to data will affect the planner at the next set_state.
        // Changes to model will only affect the physics and render threads, and
        // will not affect the planner. This is useful for studying planning
        // under model discrepancy, calls TransitionLocked and
        // InternalResidual()->Update() while holding a lock
        void Transition(mjModel *model, mjData *data);

        // get information from model
        // calls ResetLocked and InternalResidual()->Update() while holding a
        // lock
        void Reset(const mjModel *model);

        // calls CostTerms on the pointer returned from InternalResidual(),
        ...
        // calls CostValue on the pointer returned from InternalResidual(),
        // while holding a lock
    ...
    protected:
        ...
        // returns an object which can compute the residual function. the
        // function can assume that a lock on mutex_ is held when it's called
        ...
        // in some cases the transition logic requires calling mj_forward (e.g.,
        // for measuring contact forces), which will call the sensor callback,
        // which calls ResidualLocked. In order to avoid such resource
        // contention, mutex_ might be temporarily unlocked, but it must be
