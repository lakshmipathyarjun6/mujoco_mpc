        ...
        if (stage == mjSTAGE_ACC)
        {
            if (!sim->agent->allocate_enabled && sim->uiloadrequest.load() == 0)
            {
                if (sim->agent->IsPlanningModel(model))
                {
                    ...
                    const mjpc::ResidualFn *residual =
                        sim->agent->PlanningResidual();
                    residual->Residual(model, data, data->sensordata);
                }
                else
                {
                    // this residual is used by the physics thread and the UI
                    // thread (for plots), and is run with a shared lock, to
                    // safely run with changes to weights and parameters
                    sim->agent->ActiveTask()->Residual(model, data,
                                                       data->sensordata);
                }
            }
        }
            ...
            if (sim.uiloadrequest.load() == 0)
            {
                ...
                else
                {
                    ...
                    // get simulation state (lock physics thread)
                    {
                        ...
                        mju_copy(sim.agent->sensor.data(), d->sensordata,
                                 m->nsensordata);
                        ...
                        mju_copy(estimator->Data()->mocap_pos, d->mocap_pos,
                                 3 * m->nmocap);
                        mju_copy(estimator->Data()->mocap_quat, d->mocap_quat,
                                 4 * m->nmocap);

                        // copy simulation userdata
                        mju_copy(estimator->Data()->userdata, d->userdata,
                                 m->nuserdata);
                    }

                    // update filter using latest ctrl and sensor copied from
                    // physics thread
                    estimator->Update(sim.agent->ctrl.data(),
                                      sim.agent->sensor.data());

                    // estimator state to planner
                    double *state = estimator->State();
                    sim.agent->state.Set(m, state, state + m->nq,
                                         state + m->nq + m->nv, d->mocap_pos,
    ...
    void PhysicsLoop(mj::Simulate &sim)
    {
        ...
        while (!sim.exitrequest.load())
        {
            ...
            if (sim.uiloadrequest.load() == 1)
            {
                // get new model + task
                sim.filename =
                    sim.agent->GetTaskXmlPath(sim.agent->gui_task_id);
                ...
                if (dnew)
                {
                    ...
                    sim.agent->plan_enabled =
                        absl::GetFlag(FLAGS_planner_enabled);
                    sim.agent->Allocate();
                    ...
                    free(ctrlnoise);
                    ctrlnoise =
                        static_cast<mjtNum *>(malloc(sizeof(mjtNum) * m->nu));
                ...
                { // run only if model is present
                    ...
                    if (sim.run)
                    {
                        ...
                        if (sim.ctrl_noise_std)
                        {
                            ...
                            // (Ornstein–Uhlenbeck)
                            mjtNum rate =
                                mju_exp(-m->opt.timestep / sim.ctrl_noise_rate);
                            mjtNum scale =
                                sim.ctrl_noise_std * mju_sqrt(1 - rate * rate);
                        ...
                        double slowdown =
                            100 / sim.percentRealTime[sim.real_time_index];

                        // misalignment condition: distance from target sim time
                        // is bigger than maximum misalignment `syncMisalign`
                        bool misaligned =
                            mju_abs(Seconds(elapsedCPU).count() / slowdown -
                            ...
                            misaligned || sim.speed_changed)
                        {
                            ...
                            sim.ApplyPosePerturbations(
                                0); // move mocap bodies only
                            sim.ApplyForcePerturbations();

                            // run single step, let next iteration deal with
                            // timing
                        ...
                        { // in-sync: step until ahead of cpu
                            ...
                            double refreshTime =
                                simRefreshFraction / sim.refresh_rate;

                            // step while sim lags behind cpu and within
                            // refreshTime
                    ...
                    { // paused
                        ...
                        sim.ApplyPosePerturbations(
                            1); // move mocap and dynamic bodies
            ...
            if (sim.uiloadrequest.load() == 0)
            {
                ...
                if (!sim.agent->ActiveEstimatorIndex() ||
...
namespace mjpc
{
                     ...
                     int task_id)
    {
        ...
        sim = std::make_unique<mj::Simulate>(
            std::make_unique<mujoco::GlfwAdapter>(), std::make_shared<Agent>());
        ...
        auto closest = std::min_element(
            ...
            [&](float a, float b) {
                return std::abs(a - desired_percent) <
                       std::abs(b - desired_percent);
            });
        sim->real_time_index =
    ...
    mj::Simulate *MjpcApp::Sim() { return sim.get(); }
