...
#ifndef MJPC_BASISFUNCTION_H_
#define MJPC_BASISFUNCTION_H_
...
namespace mjpc
{
    ...
    public:
        ...
        {
            LogAssert(mNumControls == 0 && mDegree == 0,
                      "Object already created.");
            LogAssert(input.numControls >= 2,
                      "Invalid number of control points.");
            LogAssert(1 <= input.degree && input.degree < input.numControls,
                      "Invalid degree.");
            LogAssert(input.numUniqueKnots >= 2,
            ...
            size_t numCols = static_cast<size_t>(mNumControls) +
                             static_cast<size_t>(mDegree);
                      ...
                      int32_t &maxIndex) const
        {
            ...
            Real n0 = t - mKnots[i],
                 n1 = mKnots[static_cast<size_t>(i) + 1] - t;
            ...
            for (j = 2; j <= mDegree; ++j)
            {
                ...
                {
                    n0 = t - mKnots[k];
                    n1 = mKnots[static_cast<size_t>(k) +
                                static_cast<size_t>(j) + 1] -
                         t;
                    d0 = mKnots[static_cast<size_t>(k) +
                                static_cast<size_t>(j)] -
                         mKnots[k];
                    d1 = mKnots[static_cast<size_t>(k) +
                                static_cast<size_t>(j) + 1] -
