...
namespace mjpc
{
    ...
    void Task::Reset(const mjModel *model)
    {
        ...
        {
            mju_error("Cost construction from XML: User sensors specifying "
                      "residuals must be "
        ...
        {
            mju_error("Number of cost terms exceeds maximum. Either: 1) reduce "
                      "number of "
        ...
        for (int i = 0; i < num_term; i++)
        {
            ...
            // user data: [norm, weight, weight_lower, weight_upper,
            // parameters...]
    ...
    {
        std::lock_guard<std::mutex> lock(mutex_);
        return InternalResidual()->CostTerms(terms, residual,
                                             /*weighted=*/true);
    }
    ...
    // Does nothing by default, can be overridden in derived classes
    vector<double> Task::GetDesiredStateFromPCs(double time) const
    {
        vector<double> desired_state;
        return desired_state;
    }
    ...
    // Does nothing by default, can be overridden in derived classes
    vector<vector<double>> Task::GetBSplineControlData(
