...
namespace
{
    ...
    // file section of UI
    const mjuiDef defFile[] = {{mjITEM_SECTION, "File", 0, nullptr, "AF"},
    ...
    // number of lines in the Constraint ("Counts") and Cost ("Convergence")
    // figures
    ...
    void UpdateProfiler(mj::Simulate *sim)
    {
        ...
        for (int k = 0; k < nisland; k++)
        {
            ...
            int npoints =
                mjMIN(mjMIN(sim->d->solver_niter[k], mjNSOLVER), mjMAXLINEPNT);
            ...
            for (int i = 0; i < npoints; i++)
            {
                ...
                // y
                int nefc =
                    nisland == 1 ? sim->d->nefc : sim->d->island_efcnum[k];
                sim->figconstraint.linedata[start + 0][2 * i + 1] = nefc;
                const mjSolverStat *stat = sim->d->solver + k * mjNSOLVER + i;
                sim->figconstraint.linedata[start + 1][2 * i + 1] =
                    stat->nactive;
                sim->figconstraint.linedata[start + 2][2 * i + 1] =
                    stat->nchange;
                sim->figconstraint.linedata[start + 3][2 * i + 1] = stat->neval;
                sim->figconstraint.linedata[start + 4][2 * i + 1] =
                    stat->nupdate;
            }
        ...
        float tdata[5] = {
            ...
            static_cast<float>(sim->d->timer[mjTIMER_POS_COLLISION].duration /
                               number),
            static_cast<float>(sim->d->timer[mjTIMER_POS_MAKE].duration /
                               number) +
                static_cast<float>(sim->d->timer[mjTIMER_POS_PROJECT].duration /
                                   number),
            static_cast<float>(sim->d->timer[mjTIMER_CONSTRAINT].duration /
                               number),
            0};
        ...
        float sdata[6] = {
            static_cast<float>(sim->m->nv),   static_cast<float>(sim->m->nbody),
            static_cast<float>(sim->d->nefc), static_cast<float>(sqrt_nnz),
            static_cast<float>(sim->d->ncon), static_cast<float>(solver_niter)};
    ...
    void ShowProfiler(mj::Simulate *sim, mjrRect rect)
    {
        mjrRect viewport = {rect.left + rect.width - rect.width / 4,
                            rect.bottom, rect.width / 4, rect.height / 4};
        ...
        viewport.bottom += rect.height / 4;
        mjr_figure(viewport, &sim->figconstraint,
                        ...
                        double interval)
    {
        ...
        if (d->solver_niter[island])
        {
            ...
            const mjSolverStat *stat =
                sim->d->solver + island * mjNSOLVER + ind;
        ...
        if (best_trajectory)
        {
            ...
            mju::sprintf_arr(content, "%.3f\n%d\n%d\n%d\n%-9.3f\n%.2g of %s",
                             best_trajectory->total_return, m->nv, m->nu,
                             nparam, d->time,
...
#undef X
    ...
    void MakePhysicsSection(mj::Simulate *sim, int oldstate)
    {
        ...
        mjuiDef defPhysics[] = {
            ...
            {mjITEM_SELECT, "Jacobian", 2, &(opt->jacobian),
             "Dense\nSparse\nAuto"},
            ...
            {mjITEM_EDITINT, "Noslip Iter", 2, &(opt->noslip_iterations),
             "1 0 1000"},
            {mjITEM_EDITNUM, "Noslip Tol", 2, &(opt->noslip_tolerance),
             "1 0 1"},
        ...
        mjuiDef defFlag[] = {{mjITEM_CHECKINT, "", 2, nullptr, ""},
                             {mjITEM_END}};
    ...
    void MakeRenderingSection(mj::Simulate *sim, int oldstate)
    {
        ...
        for (int i = 0; i < mjMIN(sim->m->ncam, mjMAXUIMULTI - 2); i++)
        {
            ...
            {
                mju::strcat_arr(camname,
                                sim->m->names + sim->m->name_camadr[i]);
            }
            else
            {
            ...
            if (mju::strlen_arr(camname) +
                    mju::strlen_arr(defRendering[1].other) >=
                mjMAXUITEXT - 1)
            {
                break;
            }
        ...
        // add flags programmatically
        mjuiDef defFlag[] = {{mjITEM_CHECKBYTE, "", 2, nullptr, ""},
                             {mjITEM_END}};
    ...
    {
        mjuiDef defJoint[] = {
            {mjITEM_SECTION, "Joint", oldstate, nullptr, "AJ"}, {mjITEM_END}};
        ...
        for (int i = 0; i < sim->m->njnt && itemcnt < mjMAXUIITEM; i++)
            if ((sim->m->jnt_type[i] == mjJNT_HINGE ||
                 sim->m->jnt_type[i] == mjJNT_SLIDE))
            {
                // skip if joint group is disabled
                if (!sim->opt.jointgroup[mjMAX(
                        0, mjMIN(mjNGROUP - 1, sim->m->jnt_group[i]))])
                {
                    continue;
                }
                ...
                {
                    mju::strcpy_arr(defSlider[0].name,
                                    sim->m->names + sim->m->name_jntadr[i]);
                }
                else
                {
    ...
    void MakeControlSection(mj::Simulate *sim, int oldstate)
    {
        ...
        for (int i = 0; i < sim->m->nu && itemcnt < mjMAXUIITEM; i++)
        {
            ...
            if (!sim->opt.actuatorgroup[mjMAX(
                    0, mjMIN(mjNGROUP - 1, sim->m->actuator_group[i]))])
            {
                continue;
            }
            ...
            {
                mju::strcpy_arr(defSlider[0].name,
                                sim->m->names + sim->m->name_actuatoradr[i]);
            }
            else
            {
            ...
            if (sim->m->actuator_ctrllimited[i])
                mju::sprintf_arr(defSlider[0].other, "%.4g %.4g",
                                 sim->m->actuator_ctrlrange[2 * i],
                                 sim->m->actuator_ctrlrange[2 * i + 1]);
            else
            {
                mju::strcpy_arr(defSlider[0].other, "-1 1");
            }
    ...
    void CopyKey(mj::Simulate *sim)
    {
        ...
        {
            mju::sprintf_arr(buf, i == sim->m->nq - 1 ? "%g" : "%g ",
                             sim->d->qpos[i]);
    ...
    mjtNum Timer()
    {
        return Milliseconds(mj::Simulate::Clock::now().time_since_epoch())
            .count();
    }
    ...
    void CopyCamera(mj::Simulate *sim)
    {
        ...
        mju::sprintf_arr(clipboard,
                         "<camera pos=\"%.3f %.3f %.3f\" xyaxes=\"%.3f %.3f "
                         "%.3f %.3f %.3f %.3f\"/>\n",
    ...
    void UpdateSettings(mj::Simulate *sim)
    {
        ...
        // update UI
        mjui_update(-1, -1, &sim->ui0, &sim->uistate,
                    &sim->platform_ui->mjr_context());
    }
    ...
    {
        mjui_resize(ui, con);
        mjr_addAux(ui->auxid, ui->width, ui->maxheight, ui->spacing.samples,
                   con);
        UiLayout(state);
    ...
    void UiEvent(mjuiState *state)
    {
            ...
            state->type == mjEVENT_KEY)
        {
            ...
            if (it && it->sectionid == SECT_FILE)
            {
                ...
                case 0: // Save xml
                {
                    ...
                    if (!path.empty() &&
                        !mj_saveLastXML(path.c_str(), m, err, 200))
                    {
            ...
            else if (it && it->sectionid == SECT_SIMULATION)
            {
                ...
                case 1: // Reset
                    if (m)
                    {
                        ...
                        double *key_qpos =
                            mjpc::KeyQPosByName(sim->mnew, sim->dnew, "home");
                        if (key_qpos)
                        {
                        ...
                        double *key_qvel =
                            mjpc::KeyQVelByName(sim->mnew, sim->dnew, "home");
                        if (key_qvel)
                        {
                        ...
                        double *act =
                            mjpc::KeyActByName(sim->mnew, sim->dnew, "home");
                        if (act)
                        {
                ...
                case 6: // Load key
                    ...
                    mju_copy(d->mocap_pos, m->key_mpos + i * 3 * m->nmocap,
                             3 * m->nmocap);
                ...
                case 7: // Save key
                    ...
                    mju_copy(m->key_mpos + i * 3 * m->nmocap, d->mocap_pos,
                             3 * m->nmocap);
                    mju_copy(m->key_mquat + i * 4 * m->nmocap, d->mocap_quat,
                             4 * m->nmocap);
            ...
            {
                sim->agent->AgentEvent(it, sim->d, sim->uiloadrequest,
                                       sim->run);
            }
            ...
            //   sim->agent->EstimatorEvent(it, sim->d, sim->uiloadrequest,
            //   sim->run);
            // }
            ...
            else if (it && it->sectionid == SECT_RENDERING)
            {
                ...
                else if (sim->camera == 1)
                {
                    ...
                    else
                    {
                        sim->cam.type = mjCAMERA_FREE;
                        sim->camera = 0;
                        mjui_update(SECT_RENDERING, -1, &sim->ui0,
                                    &sim->uistate,
                                    &sim->platform_ui->mjr_context());
                    }
                }
            ...
            state->type == mjEVENT_KEY)
        {
            ...
            if (it && it->sectionid == SECT_CONTROL)
            {
                ...
                if (it->itemid == 0)
                {
                    ...
                    mjui_update(SECT_CONTROL, -1, &sim->ui1, &sim->uistate,
                                &sim->platform_ui->mjr_context());
                }
            }
        ...
        if (state->type == mjEVENT_KEY && state->key != 0)
        {
            ...
            case ']': // cycle up fixed cameras
                ...
                {
                    sim->cam.type = mjCAMERA_FIXED;
                    // simulate->camera = {0 or 1} are reserved for the free and
                    // tracking cameras
            ...
            case '-': // slow down
            {
                int numclicks = sizeof(sim->percentRealTime) /
                                sizeof(sim->percentRealTime[0]);
        ...
        if (state->type == mjEVENT_SCROLL && state->mouserect == 3 && m)
        {
            ...
            mjv_moveCamera(m, mjMOUSE_ZOOM, 0, -zoom_increment * state->sy,
                           &sim->scn, &sim->cam);
        ...
        if (state->type == mjEVENT_PRESS && state->mouserect == 3 && m)
        {
            ...
            if (state->doubleclick)
            {
                ...
                int selbody = mjv_select(
                    m, d, &sim->opt, static_cast<mjtNum>(r.width) / r.height,
                    (state->x - r.left) / r.width,
                    (state->y - r.bottom) / r.height, &sim->scn, selpnt,
                ...
                else
                {
                    if (selbody >= 0)
                    {
                        ...
                        mju_mulMatTVec(sim->pert.localpos,
                                       d->xmat + 9 * sim->pert.select, tmp, 3,
                                       3);
                    }
                    else
                    {
        ...
        if (state->type == mjEVENT_MOVE && state->dragrect == 3 && m)
        {
            ...
            mjrRect r = state->rect[3];
            if (sim->pert.active)
                mjv_movePerturb(m, d, action, state->dx / r.height,
                                -state->dy / r.height, &sim->scn, &sim->pert);
            else
                mjv_moveCamera(m, action, state->dx / r.height,
...
namespace mujoco
{
    ...
    Simulate::Simulate(std::unique_ptr<PlatformUIAdapter> platform_ui,
        ...
        : platform_ui(std::move(platform_ui)),
          uistate(this->platform_ui->state()), agent(std::move(a))
    {
    }
    ...
    void Simulate::ApplyPosePerturbations(int flg_paused)
    {
        if (this->m != nullptr)
        {
            mjv_applyPerturbPose(this->m, this->d, &this->pert,
                                 flg_paused); // move mocap bodies only
        }
    }
                        ...
                        bool delete_old_m_d)
    {
...

        {
            ...
            cond_loadrequest.wait(lock,
                                  [this]() { return this->loadrequest == 0; });
        }
    }
    ...
    void Simulate::LoadOnRenderThread()
    {
        ...
        // update scene
        mjv_updateScene(this->m, this->d, &this->opt, &this->pert, &this->cam,
                        mjCAT_ALL, &this->scn);
    ...
    void Simulate::PrepareScene()
    {
        ...
        if (this->info)
        {
            UpdateInfoText(this, this->info_title, this->info_content,
                           interval);
        }
        ...
        {
            if (!this->agent->allocate_enabled &&
                this->uiloadrequest.load() == 0)
            {
                mjui_update(SECT_TASK, -1, &this->ui0, &this->uistate,
                            &this->platform_ui->mjr_context());
            }
        }
        ...
        {
            mjui_update(SECT_AGENT, -1, &this->ui0, &this->uistate,
                        &this->platform_ui->mjr_context());
        }
    ...
    {
        if (this->platform_ui->RefreshMjrContext(this->m,
                                                 50 * (this->font + 1)))
        {
            UiModify(&this->ui0, &this->uistate,
                     &this->platform_ui->mjr_context());
            UiModify(&this->ui1, &this->uistate,
                     &this->platform_ui->mjr_context());
        }
        ...
        if (!this->m)
        {
            ...
            if (this->loadrequest)
            {
                mjr_overlay(mjFONT_BIG, mjGRID_TOPRIGHT, smallrect, "loading",
            ...
            else
            {
                ...
                mju::sprintf_arr(
                    ...
                    mj_versionString());
                mjr_overlay(mjFONT_NORMAL, mjGRID_TOPLEFT, rect, intro_message,
                            0, &this->platform_ui->mjr_context());
            }
        ...
        if (this->uiloadrequest.load() == 0)
        {
            ...
            {
                this->agent->ActiveTask()->ModifyScene(this->m, this->d,
                                                       &this->scn);
            }
            // common to all tasks
        ...
        {
            mjr_overlay(mjFONT_NORMAL, mjGRID_BOTTOMLEFT, rect,
                        this->load_error, 0, &this->platform_ui->mjr_context());
        }
        ...
        if (this->help)
        {
            mjr_overlay(mjFONT_NORMAL, mjGRID_TOPLEFT, rect, help_title,
                        help_content, &this->platform_ui->mjr_context());
        }
        ...
        {
            mjr_overlay(mjFONT_NORMAL, mjGRID_BOTTOMLEFT, rect,
                        this->info_title, this->info_content,
                        &this->platform_ui->mjr_context());
        }
    ...
    void Simulate::Reset(mjModel *m, mjData *d)
    {
        ...
        // set initial qpos via keyframe
        double *key_qpos = mjpc::KeyQPosByName(this->mnew, this->dnew, "home");
        if (key_qpos)
        {
            mju_copy(this->dnew->qpos, key_qpos, this->mnew->nq);
        }
        // set initial qvel via keyframe
        double *key_qvel = mjpc::KeyQVelByName(this->mnew, this->dnew, "home");
        if (key_qvel)
        {
            mju_copy(this->dnew->qvel, key_qvel, this->mnew->nv);
        }
        // set initial act via keyframe
        double *act = mjpc::KeyActByName(this->mnew, this->dnew, "home");
        if (act)
        {
            mju_copy(this->dnew->act, act, this->mnew->na);
        }

        this->agent->Reset();
        this->agent->PlotReset();
    }
