...
namespace mjpc
{
    ...
    void Trajectory::Reset(int T, const double *initial_repeated_action)
    {
        ...
        if (initial_repeated_action != nullptr)
        {
            for (int i = 0; i < T; ++i)
            {
                mju_copy(actions.data() + i * dim_action,
                         initial_repeated_action, dim_action);
            }
        }
        else
        {
    ...
    void Trajectory::Rollout(
        std::function<void(double *action, const double *state, double time)>
            policy,
        const Task *task, const mjModel *model, mjData *data,
        const double *state, double time, const double *mocap,
    ...
    void Trajectory::NoisyRollout(
        ...
        const Task *task, const mjModel *model, mjData *data,
        const double *state, double time, const double *mocap,
        const double *userdata, double xfrc_std, double xfrc_rate, int steps)
    {
        // reset failure flag
        failure = false;
        ...
        {
            // set action
            policy(DataAt(actions, t * nu), DataAt(states, t * dim_state),
                   data->time);
            ...
            mju_copy(DataAt(states, (t + 1) * dim_state + nq + nv), data->act,
                     na);
            times[t + 1] = data->time;
        }
        ...
        // final residual
        mju_copy(DataAt(residual, (horizon - 1) * dim_residual),
                 data->sensordata, dim_residual);

        // final trace
        GetTraces(DataAt(trace, (horizon - 1) * 3 * task->num_trace), model,
                  data, task->num_trace);
    ...
    void Trajectory::UpdateReturn(const Task *task)
    {
        ...
        for (int t = 0; t < horizon; t++)
        {
            // compute stage cost
            costs[t] =
                task->CostValue(DataAt(residual, t * task->num_residual));
